import { CanvasHTMLAttributes, JSX, useCallback, useEffect, useRef, useState } from 'react'
import {
  DrawingUtils,
  FaceLandmarker,
  FilesetResolver,
...
import task from './assets/task-vision/face_landmarker.task?url'

async function loadModel() {
  const resolver = await FilesetResolver.forVisionTasks(
    new URL('./assets/task-vision/wasm', import.meta.url).href
  )
...
function drawLandMark(results: FaceLandmarkerResult, context: CanvasRenderingContext2D | null) {
  if (!context) return
...
function detectFrame(video: HTMLVideoElement, canvas: HTMLCanvasElement, faceLandmarker: FaceLandmarker, minThresholdBeforeEyesOpen: number, maxThresholdBeforeEyesClose: number, onEyesClose: () => void, onEyesOpen: () => void): void {
...
export function Camera({
  ...
  useEffect(() => {
    ...
    navigator.mediaDevices.getUserMedia({ video: true }).then(mediaStream => video.srcObject = mediaStream);
    ...
    const detectVideo = () => {
      if (isCanvasReady && faceLandmarker !== null) {
        detectFrame(video, canvas, faceLandmarker, minThresholdBeforeEyesOpen, maxThresholdBeforeEyesClose, onEyesClose, onEyesOpen)
      }
